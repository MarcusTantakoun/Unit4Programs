...
public class SearchExample1 {
    ...
    public static int binarySearch(Object[] a, Object searchValue) {
        ...
        while (left <= right) {
            ...
            int result = ((Comparable) a[midpoint]).compareTo(searchValue); //comparable
            tries++;
            if (result == 0) {
    ...
    //returns location where it is found
    public static int linearSearch(String arr[], String item) {
        for (int i = 0; i < arr.length; i++) {
            if (item.equals(arr[i])) {
                System.out.println("Linear took " + i + " tries to return");
                return i; //returns int
            }
        }
        System.out.println("Linear took " + arr.length + " tries to return");
        return -1; //if we get back -1, it means it was not found
    }
